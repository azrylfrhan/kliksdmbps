...
Deno.serve(async (req: Request) => {
  ...
  if (endpoint === "pengumuman") {
    ...
    if (method === "PUT") {
      const auth = await validateAdmin(req, supabase);
      if (!auth.valid) return errorResponse("Unauthorized", 401);
      const { id, title, content } = body as { id: number; title: string; content: string };
      if (!id || !title || !content) return errorResponse("id, title, content wajib");
      const { data, error } = await supabase.from("pengumuman").update({ title, content }).eq("id", id).select().single();
      if (error) return errorResponse(error.message);
      return jsonResponse(data);
    }
  }
  ...
  if (endpoint === "kp_data") {
    if (method === "GET") {
      ...
      // Hanya filter jika month ada dan bukan 'all'
    ...
    if (method === "PUT") {
      ...
      const { id, name, nip, old_rank, new_rank, tanggal_usulan, drive_url } = body as {
        id: number; name: string; nip: string; old_rank: string;
        new_rank: string; tanggal_usulan?: string; drive_url?: string;
      };
      if (!id || !name || !nip || !old_rank || !new_rank) {
        return errorResponse("Field wajib: id, name, nip, old_rank, new_rank");
      }

      const { data, error } = await supabase
        .from("kp_data")
        .update({ name, nip, old_rank, new_rank, tanggal_usulan: tanggal_usulan || null, drive_url: drive_url || null })
        .eq("id", id)
        .select()
        .single();
      if (error) return errorResponse(error.message);
  ...
  if (endpoint === "kp_jadwal") {
    ...
    if (method === "PUT") {
      const auth = await validateAdmin(req, supabase);
      if (!auth.valid) return errorResponse("Unauthorized", 401);
      const { id, periode, tanggal, keterangan, pdfUrl } = body as { id: number; periode: string; tanggal?: string; keterangan?: string; pdfUrl?: string };
      if (!id || !periode) return errorResponse("id dan periode wajib");
      const { data, error } = await supabase.from("kp_jadwal").update({ periode, tanggal: tanggal || null, keterangan: keterangan || null, pdf_url: pdfUrl || null }).eq("id", id).select().single();
      if (error) return errorResponse(error.message);
      return jsonResponse(data);
    }
  }
  ...
  if (endpoint === "kgb_data") {
    ...
    if (method === "PUT") {
      const auth = await validateAdmin(req, supabase);
      if (!auth.valid) return errorResponse("Unauthorized", 401);
      const { id, nama, kabkota, bulan } = body as { id: string; nama: string; kabkota: string; bulan: string };
      if (!id || !nama || !kabkota || !bulan) return errorResponse("id, nama, kabkota, bulan wajib");
      const { data, error } = await supabase.from("kgb_data").update({ nama, kabkota, bulan }).eq("id", id).select().single();
      if (error) return errorResponse(error.message);
      return jsonResponse(data);
    }
  }
  ...
  if (endpoint === "uji_kompetensi") {
    ...
    if (method === "PUT") {
      const auth = await validateAdmin(req, supabase);
      if (!auth.valid) return errorResponse("Unauthorized", 401);
      const { id, jenis, periode, name, nip, tanggal, status, keterangan } = body as { id: number; jenis: string; periode: string; name: string; nip: string; tanggal?: string; status?: string; keterangan?: string };
      if (!id || !jenis || !periode || !name || !nip) return errorResponse("id, jenis, periode, name, nip wajib");
      const { data, error } = await supabase.from("uji_kompetensi").update({ jenis, periode, name, nip, tanggal: tanggal || null, status: status || "Belum", keterangan: keterangan || null }).eq("id", id).select().single();
      if (error) return errorResponse(error.message);
      return jsonResponse(data);
    }
  }
  ...
  if (endpoint === "peraturan") {
    ...
    if (method === "PUT") {
      const auth = await validateAdmin(req, supabase);
      if (!auth.valid) return errorResponse("Unauthorized", 401);
      const { id, judul, nomor, tahun, kategori, deskripsi, url: docUrl } = body as { id: number; judul: string; nomor?: string; tahun?: string; kategori?: string; deskripsi?: string; url?: string };
      if (!id || !judul) return errorResponse("id dan judul wajib");
      const { data, error } = await supabase.from("peraturan").update({ judul, nomor: nomor || null, tahun: tahun || null, kategori: kategori || null, deskripsi: deskripsi || null, url: docUrl || null }).eq("id", id).select().single();
      if (error) return errorResponse(error.message);
      return jsonResponse(data);
    }
  }
  ...
  if (endpoint === "tb_persyaratan") {
    ...
    if (method === "PUT") {
      const auth = await validateAdmin(req, supabase);
      if (!auth.valid) return errorResponse("Unauthorized", 401);
      const { id, title, description, url: docUrl } = body as { id: number; title: string; description?: string; url: string };
      if (!id || !title || !docUrl) return errorResponse("id, title, url wajib");
      const { data, error } = await supabase.from("tugas_belajar_persyaratan").update({ title, description: description || null, url: docUrl }).eq("id", id).select().single();
      if (error) return errorResponse(error.message);
      return jsonResponse(data);
    }
  }
  ...
  if (endpoint === "tb_dokumen") {
    if (method === "GET") {
      const { data, error } = await supabase.from("tugas_belajar_dokumen").select("id, title, description, url").order("sort_order", { ascending: true });
      if (error) return errorResponse(error.message);
      return jsonResponse(data || []);
    }
    if (method === "POST") {
      const auth = await validateAdmin(req, supabase);
      if (!auth.valid) return errorResponse("Unauthorized", 401);
      const { title, description, url: docUrl } = body as { title: string; description?: string; url: string; };
      if (!title || !docUrl) return errorResponse("title dan url wajib");
      const { count } = await supabase.from("tugas_belajar_dokumen").select("*", { count: "exact", head: true });
      const { data, error } = await supabase.from("tugas_belajar_dokumen").insert({ title, description: description || null, url: docUrl, sort_order: (count || 0) + 1 }).select().single();
      if (error) return errorResponse(error.message);
      return jsonResponse(data, 201);
    }
    if (method === "DELETE") {
    ...
    if (method === "PUT") {
      ...
      const { id, title, description, url: docUrl } = body as { id: number; title: string; description?: string; url: string };
      if (!id || !title || !docUrl) return errorResponse("id, title, url wajib");
      const { data, error } = await supabase.from("tugas_belajar_dokumen").update({ title, description: description || null, url: docUrl }).eq("id", id).select().single();
      if (error) return errorResponse(error.message);
      return jsonResponse(data);
    }
  }

  // Helper PUT untuk tabel title/description/url
  const docTableMap: Record<string, string> = {
    "karis_karsu_persyaratan": "karis_karsu_persyaratan",
    "karis_karsu_dokumen": "karis_karsu_dokumen",
    "perkawinan_pertama_persyaratan": "perkawinan_pertama_persyaratan",
    "perkawinan_pertama_dokumen": "perkawinan_pertama_dokumen",
    "pensiun_persyaratan": "pensiun_persyaratan",
    "pensiun_dokumen": "pensiun_dokumen",
  };

  if (docTableMap[endpoint]) {
    const tbl = docTableMap[endpoint];
    if (method === "GET") {
      const { data, error } = await supabase.from(tbl).select("id, title, description, url").order("sort_order", { ascending: true });
      if (error) return errorResponse(error.message);
      return jsonResponse(data || []);
    }
    if (method === "POST") {
      ...
      const { data, error } = await supabase.from(tbl).insert({ title, description: description || null, url: docUrl, sort_order: (count || 0) + 1 }).select().single();
      if (error) return errorResponse(error.message);
      return jsonResponse(data, 201);
    }
    if (method === "DELETE") {
    ...
    if (method === "PUT") {
      ...
      const { id, title, description, url: docUrl } = body as { id: number; title: string; description?: string; url: string };
      if (!id || !title || !docUrl) return errorResponse("id, title, url wajib");
      const { data, error } = await supabase.from(tbl).update({ title, description: description || null, url: docUrl }).eq("id", id).select().single();
      if (error) return errorResponse(error.message);
      return jsonResponse(data);
    }
  }
